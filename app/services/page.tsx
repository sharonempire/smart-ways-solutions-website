...
export const metadata: Metadata = {
  ...
  description: "Home loans, car loans, takeover + top-up loans, loan against property, and business loans in Kerala — facilitated by Smart Way Solutions across 16+ banks and NBFCs.",
...
export default function ServicesPage() {
  return (
    <>
      ...
      {/* ── Car Loans ──────────────────────────────── */}
      <section id="car-loans" className="py-20 px-6 bg-white scroll-mt-20">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-12 h-12 rounded-2xl bg-[#F5A623] flex items-center justify-center text-2xl shrink-0">🚗</div>
            <div>
              <p className="text-[#F5A623] text-xs font-bold uppercase tracking-widest">Category 05</p>
              <h2 className="text-[#1a1a1a] text-3xl font-black">Car Loans</h2>
            </div>
          </div>
          <p className="text-gray-600 text-sm max-w-2xl mb-10 leading-relaxed">
            Whether you&apos;re buying brand-new or pre-owned, we source the best car loan rates from 16+ banks and NBFCs across Kerala. Fast approvals, minimal paperwork, and we handle everything.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
            {carLoans.map((loan) => (
              <div
                key={loan.type}
                className={`rounded-2xl p-7 border-2 relative overflow-hidden ${
                  loan.highlight
              ...
              >
                {loan.highlight && (
                  <div className="absolute top-0 right-0 bg-[#F5A623] text-black text-[10px] font-black px-3 py-1 rounded-bl-xl">
                    Most Popular
                  </div>
                )}
                <div className="w-10 h-10 rounded-xl bg-[#F5A623]/15 flex items-center justify-center text-xl mb-4">
                ...
                <p className="text-[#F5A623] font-black text-xs uppercase tracking-widest mb-1">{loan.badge}</p>
                <h3 className="text-[#1a1a1a] font-black text-xl mb-3">{loan.title}</h3>
                <p className="text-gray-600 text-sm leading-relaxed mb-5">{loan.description}</p>
                <ul className="space-y-2 mb-6">
                  {loan.features.map((f) => (
                    <li key={f} className="flex items-center gap-2.5 text-xs text-gray-600">
                      <span className="w-4 h-4 rounded-full bg-[#F5A623]/15 text-[#d4891a] flex items-center justify-center text-[10px] font-black shrink-0">✓</span>
                ...
                <Link
                  href="/enquire"
                  className={`inline-block px-6 py-2.5 rounded-xl font-black text-sm transition-colors ${
                    loan.highlight
          ...
          {/* Quick comparison */}
          <div className="bg-[#1a1a1a] rounded-2xl p-8 text-white">
            <p className="text-[#F5A623] font-bold text-xs uppercase tracking-widest mb-5">New vs Used — Quick Comparison</p>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-left text-gray-400 font-semibold pb-3 pr-6">Feature</th>
                    <th className="text-left text-[#F5A623] font-black pb-3 pr-6">New Car</th>
                    <th className="text-left text-gray-300 font-black pb-3">Used Car</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {[
                    { feature: "Interest Rate", new: "7.5%–9.5% p.a.", used: "9%–14% p.a." },
                    { feature: "Max Financing", new: "Up to 100% on-road", used: "Up to 90% of value" },
                    { feature: "Max Tenure", new: "7 years", used: "5 years" },
                    { feature: "Vehicle Age", new: "Brand new", used: "Up to 10 years old" },
                    { feature: "Processing Time", new: "1–2 working days", used: "2–3 working days" },
                  ].map((row) => (
                    <tr key={row.feature}>
                      <td className="text-gray-400 py-3 pr-6">{row.feature}</td>
                      <td className="text-[#F5A623] font-bold py-3 pr-6">{row.new}</td>
                      <td className="text-gray-300 py-3">{row.used}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <p className="text-gray-500 text-[10px] mt-4">Rates indicative. Final rate depends on credit profile, lender, and vehicle value.</p>
          </div>
        </div>
      </section>
